...
void Key_Process(void)
{
	...
	{
		u8 i=0;
		if((Pass_Now[0]==Pass_data[0])&&(Pass_Now[1]==Pass_data[1])&&(Pass_Now[2]==Pass_data[2]))
		{
			Interface=1;
		}
		else
...
void LCD_Process(void)
{
	...
	if(Interface==0)
	{
	   ...
	   LCD_DisplayStringLine(Line1,display_buf);
		...
		sprintf((char*)display_buf,"    B1:%d",Pass_Now[0]);
	   LCD_DisplayStringLine(Line3,display_buf);
		
		sprintf((char*)display_buf,"    B2:%d",Pass_Now[1]);
	   LCD_DisplayStringLine(Line4,display_buf);
		
		sprintf((char*)display_buf,"    B3:%d",Pass_Now[2]);
